...
def test_login_ldap_ui_form_not_valid():
    ...
    @patch('invenio_ldapclient.views.login_form_factory',
           mock_login_form_factory)
    def inner(mock_render_template):
        from invenio_ldapclient.views import login_ldap_ui

        login_ldap_ui()
        mock_render_template.assert_called_with('some_template.html',
                                                login_user_form = mock_form)
...
def test_login_ldap_ui_form_valid_no_entry():
    ...
    @patch('invenio_ldapclient.views.login_form_factory',
           mock_login_form_factory)
    @patch('invenio_ldapclient.views.check_dit_fetch_entries',
           mock_check_dit_fetch_entries)
    def inner(mock_render_template):
        from invenio_ldapclient.views import login_ldap_ui
