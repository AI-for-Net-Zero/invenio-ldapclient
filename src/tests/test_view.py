...
def test_login_add_user(configured_app,
                        mock_server_factory,
                        mock_login_form_factory_factory,
                        mock_user_factory):
    app = configured_app
    ...
    '''
    Not ideal - called twice, 1st time in db.find_or_register_user (returns None) then 2nd
    time after calling _datastore.create_user(...)
    '''
    mock_one_or_none = Mock(one_or_none = Mock(side_effect = [None, mock_user]))
    def _filter_by(username):
        return mock_one_or_none
